...
Let us think about why Geoffrey Hinton for so many decades believed that neural networks could mimic decisions made by the brain. Lets take an example about a buyer rating a car from a scale of 0-10. There would be some factors like price, mileage, pick-up, comfort, brand etc that a buyer would look at to rate a car. There is a different degree of importance he places on each of the factors. Now, there must be some internal function in the buyer's mind, unbeknown to him, in which he takes in these factors and computes his output score. This function is very approximately linear. I.e he assigns a fixed score increase for every fixed step increase in the factors. Example if mileage changes by 2km then he always adds 0.5 to his score. Think about even the way you may rate your favorite sports player, it is very approximately linear except maybe at the very hign and low ends of the scale. As we know this can be represented by a single neuron. But now suppose I told you to rate sports cars, your function would change alot. You may not place the same importance on brand and alot more to the stats of the car. The same if I told you to rate a jeep, buget or luxuary cars. What you have is a whole collection of approximately linear internal functions ready at your disposal according to the situation. We can represent these many functions as a layer in a neural network. Now if I show you just pictures of random cars, you would first decide which type of car this is and the use the rating function assoicated with that. So there a an extra layer of preprocessing you do. We can represent this is layer 1 and the rating functions of the car as layer 2. So we have a type of nested funtion.
...
<img src="https://github.com/bluesky314/bluesky314.github.io/blob/master/images/TFplaygroundc.png?raw=true" width="850" height="300" >
 ...
 Just click play and you'll see a solution being formed. The network forms a different solution each time depending upon how the weights are initialized. In the first one, 3 perceptrons corner the Blue region from three sides and the final node(not explicitly shown) applies an AND gate to all the three. The second found an inverse region to apply a NOT gate to. The sign of the weights can be made out from the color of the connections shown and the weights can be seen by clicking each perceptron.
